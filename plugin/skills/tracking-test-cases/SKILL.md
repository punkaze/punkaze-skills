...
- **Notify:** the server exits on either footer button (Submit or Done & close), no polling. The run file's `action` field tells you which: `"submit"` means relaunch for another round, `"done"` means this QA pass is finished.
